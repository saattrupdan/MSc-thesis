...
\title{{\scshape\small Master's Defense}\\ Inner Model Theory}
\author{Dan Saattrup Nielsen}
\date{\tiny August 30, 2016}
...
A (coarse) \textbf{premouse} is a structure of the form $\M=\bra{J_\alpha^{\E},\E,F}$, where $\E$ is a {\color{magenta}fine} extender sequence and every proper initial segment of $\M$ is {\color{orange}sound}.\\\pause
\end{block}

\begin{center}
\includegraphics[scale=0.11]{gfx/premouse.png}\hspace{0.5cm}
\begin{tikzpicture}
...
\pause {\small If $\M\models\zf^-$ then $\core(\M)=\M$. {\color{orange}Soundness} implies that $\core(\M)=\M$.}
...
\begin{tikzpicture}
  ...
  \onslide<3-13>
  \node at (5,3.5) {\parbox{7cm}{All extenders used are {\color{teal}robust}.}};
